...
IMAGE_SIZE = (512, 512)
NUM_ANIMATION_FRAMES = 120 # Number of frames for the animation loop
...
# --- Helper Functions (retained from previous scripts) ---
...
def generate_elliptical_path(pca_scores: np.ndarray, num_points: int):
    print(f"Generating elliptical path for {num_points} frames...")
    
    # We will use the first two principal components to define the ellipse
    pca_scores_2d = pca_scores[:, :2]
    
    # Center the data
    center = np.mean(pca_scores_2d, axis=0)
    centered_scores = pca_scores_2d - center
    
    # Compute the covariance matrix and its eigenvectors/eigenvalues
    cov_matrix = np.cov(centered_scores, rowvar=False)
    ...
    t = np.linspace(0, 2 * np.pi, num_points, endpoint=False) # Exclude endpoint for seamless loop
    
    # Parametric equations for the ellipse
    x_ellipse = semi_major_axis * np.cos(t)
    y_ellipse = semi_minor_axis * np.sin(t)
    
    # Rotate the ellipse to align with the data's principal axes
    rotated_points = np.dot(np.column_stack((x_ellipse, y_ellipse)), eigenvectors.T)
    
    # Translate back to the original center
    final_path_2d = rotated_points + center
    
    # Pad with zeros for the remaining PCA dimensions
    num_other_dims = pca_scores.shape[1] - 2
    if num_other_dims > 0:
        other_dims = np.zeros((num_points, num_other_dims))
        final_path = np.hstack((final_path_2d, other_dims))
    else:
        final_path = final_path_2d
        
    print("Elliptical path generated successfully.")
    return final_path
...
def create_animation_frames():
    ...
    # 2. Generate PCA scores along the elliptical path
    pca_scores_path = generate_elliptical_path(pca_data['original_pca_scores'], NUM_ANIMATION_FRAMES)
    
    # 3. Loop through each frame and generate the 6-panel plot
    print(f"\nGenerating {NUM_ANIMATION_FRAMES} frames...")
    for i, pca_score in enumerate(pca_scores_path):
        frame_num = i + 1
        print(f"Processing frame {frame_num}/{NUM_ANIMATION_FRAMES}...")
        
        # Inverse Transform PCA to get coordinates
        ...
        # Assemble and save the final 6-panel plot
        ...
        plt.tight_layout()
        output_path = PLOTS_DIR / f"frame_{i:03d}.png"
        plt.savefig(output_path, dpi=100)
        plt.close(fig)

    print("\nAll animation frames saved successfully.")
